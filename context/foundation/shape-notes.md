...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "primary persona"
      decision: "professionals learning on the job — domain knowledge tied to work"
    - topic: "pain type"
      decision: "workflow friction — the task is possible but too slow"
    - topic: "input shape"
      decision: "pasted text only — user copies content from anywhere"
    - topic: "auth method"
      decision: "email + password login; no OAuth"
    - topic: "role model"
      decision: "flat — all logged-in users have the same capabilities"
    - topic: "SRS integration"
      decision: "third-party SRS service / API — scheduling delegated externally"
    - topic: "MVP timeline"
      decision: "3 weeks after-hours"
    - topic: "deck model"
      decision: "named decks — each generation run targets a deck; cards belong to a deck"
    - topic: "post-accept editing"
      decision: "user can edit a card after it has been added to a deck"
  frs_drafted: 14
...
MVP first flow (end-to-end, 3 weeks after-hours):
1. User signs up / logs in (email + password)
2. User pastes text (study material)
3. App calls AI to generate flashcards from the text into a named deck
4. User reviews each generated card — accepts or rejects
5. Accepted cards are saved to the deck
6. User starts a spaced repetition review session; scheduling is handled by a third-party SRS service
...
- Any operation that takes longer than two seconds provides continuous visible progress feedback to the user.
...
- **No mobile app or mobile-optimized experience.** Desktop browsers only for the MVP. Rationale: explicitly out of scope per product notes; mobile adds UI complexity with no near-term payoff.
